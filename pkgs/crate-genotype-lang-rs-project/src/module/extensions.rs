        ...
        loop {
            ...
            let resolved_definition_id = match definition {
                Some((current_definition_id, (span, referenced_ids))) => {
                                ...
                                RsDefinition::Struct(reference_struct) => {
                                    match &reference_struct.fields {
                                        ...
                                        RsStructFields::Unresolved(span, _, _) => {
                                            ...
                                            Err(RsProjectError::ExtensionResolve(
                                                *span,
                    ...
                    let project_module = modules
                        .values_mut()
                        .filter_map(|module_state| module_state.project_module_mut())
                        .find(|module_state| module_state.module.id == current_definition_id.0)
                        .ok_or_else(|| {
                            RsProjectError::ExtensionResolve(
                                *span,
                                format!(
                    ...
                    let cleared_extension_references = project_module
                        .module
                        .definitions
                        .iter_mut()
                        ...
                        .and_then(|definition| {
                            if let RsDefinition::Struct(r#struct) = definition {
                                match &r#struct.fields {
                                    ...
                                    _ => Err(RsProjectError::ExtensionResolve(
                                        *span,
                    ...
                    for (reference_id, definition_id) in visitor.pulled_references.clone() {
                        let pulled_definition_resolve = project_module
                            .project_resolve
                            .definitions
                            .entry(definition_id)
                    ...
                    for (_, referenced_definition_id) in visitor.pulled_references {
                        ...
                        let existing_use =
                            project_module.module.imports.iter_mut().find(|import| {
                        ...
                        match existing_use {
                            ...
                            None => {
                                ...
                                project_module.module.imports.push(RsUse {
                    ...
                    // Clean up references
                    project_module.module.imports.iter_mut().for_each(|r#use| {
                        if let RsDependencyIdent::Local(path) = &r#use.dependency
                            && let RsUseReference::Named(names) = &r#use.reference
                        {
                            let mut names = names.clone();
                            names.retain(|name| {
                                let resolve = project_module.project_resolve.definitions.get(
...
impl RsProjectModule {
    pub fn resolve_extensions(
        ...
        RsProjectModule::expand_extension_fields(modules).map_err(GtlProjectError::resolve)
    }
}
