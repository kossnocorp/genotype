...
impl Default for GtpConfigSetVersionProps {
    fn default() -> Self {
        Self {
...
impl GtpConfig {
    pub fn set_manifest_version(&mut self, props: GtpConfigSetVersionProps) -> Result<()> {
        let GtpConfigSetVersionProps {
            version,
            ts,
            py,
            rs,
        ...
        if let Some(cur_ts_version) = self.lang_manifest_version(GtpLang::Ts)? {
            ensure_can_set_version(&cur_ts_version, &version)?;
            let ts_version = ts.as_ref().unwrap_or(&version);
            updates.insert(
                GtpConfigVersionTarget::Lang(GtpLang::Ts),
                ts_version.clone(),
            );
        }

        if let Some(cur_py_version) = self.lang_manifest_version(GtpLang::Py)? {
            ensure_can_set_version(&cur_py_version, &version)?;
            let py_version = py.as_ref().unwrap_or(&version);
            updates.insert(
                GtpConfigVersionTarget::Lang(GtpLang::Py),
                py_version.clone(),
            );
        }

        if let Some(cur_rs_version) = self.lang_manifest_version(GtpLang::Rs)? {
            ensure_can_set_version(&cur_rs_version, &version)?;
            let rs_version = rs.as_ref().unwrap_or(&version);
            updates.insert(
                GtpConfigVersionTarget::Lang(GtpLang::Rs),
                rs_version.clone(),
            );
        }
    ...
    pub fn bump_manifest_version(&mut self, part: GtpConfigVersionPart) -> Result<()> {
        ...
        if updates.is_empty() {
            return Err(GtpConfigError::ManifestVersionMissingForBump.into());
        }
    ...
    fn apply_version_updates(&mut self, updates: VersionUpdates) -> Result<()> {
        for (target, next_version) in updates.into_iter() {
            match target {
                GtpConfigVersionTarget::Global => {
                    self.version = Some(next_version.clone());
                }

                GtpConfigVersionTarget::Lang(lang) => {
    ...
    pub fn lang_manifest_mut(&mut self, lang: GtpLang) -> &mut toml::Table {
        match lang {
            GtpLang::Ts => &mut self.ts.common.manifest,

            GtpLang::Py => &mut self.py.common.manifest,

            GtpLang::Rs => &mut self.rs.common.manifest,
        }
    }

    pub fn lang_manifest(&self, lang: GtpLang) -> &toml::Table {
        match lang {
            GtpLang::Ts => &self.ts.common.manifest,

            GtpLang::Py => &self.py.common.manifest,

            GtpLang::Rs => &self.rs.common.manifest,
        }
    }

    pub fn lang_manifest_version(&self, lang: GtpLang) -> Result<Option<Version>> {
...
fn bump_version(version: &Version, part: GtpConfigVersionPart) -> Version {
    let mut next = version.clone();
    match part {
        GtpConfigVersionPart::Major => {
            next.major += 1;
            next.minor = 0;
            next.patch = 0;
        }

        GtpConfigVersionPart::Minor => {
            next.minor += 1;
            next.patch = 0;
        }

        GtpConfigVersionPart::Patch => {
            next.patch += 1;
        }
    }
    next
...
impl GtpConfig {
    fn target_version_path(&self, lang: GtpLang) -> &'static str {
        match lang {
            GtpLang::Ts => TS_MANIFEST_VERSION_PATH,
            GtpLang::Py => match self.py.lang.manager {
                PyPackageManager::Poetry => PY_MANIFEST_VERSION_PATH_POETRY,
                PyPackageManager::Uv => PY_MANIFEST_VERSION_PATH_UV,
            },
            GtpLang::Rs => RS_MANIFEST_VERSION_PATH,
        }
    }
}
...
fn ensure_can_set_version(current: &Version, next: &Version) -> Result<()> {
    if next < current {
        return Err(GtpConfigError::VersionLower {
...
fn parse_target_version_toml_val(target: &toml::Value) -> Result<Version> {
    let value = target
        .as_str()
        .ok_or_else(|| GtpConfigError::VersionInvalid(target.to_string()))
        .into_diagnostic()?;
...
fn parse_version_str(value: &str) -> Result<Version> {
    let version = Version::parse(value)
        .map_err(|_| GtpConfigError::VersionInvalid(value.into()))
        .into_diagnostic()?;

    if !version.pre.is_empty() || !version.build.is_empty() {
        return Err(GtpConfigError::VersionInvalid(value.into())).into_diagnostic();
    }
...
mod tests {
    ...
    fn test_set_global_version() {
        let mut config = GtpConfig::from_toml_str(
            r#"
version = "0.1.0"
...
"#,
        ...
        config
            .bump_manifest_version(GtpConfigVersionPart::Minor)
            .unwrap();
