...
impl GtConfig {
    pub fn from_toml_str(source: &str) -> Result<Self> {
        let mut config: GtConfig =
            Figment::from(figment::providers::Serialized::defaults(GtConfig::default()))
    ...
    pub fn to_toml_str(&self) -> Result<String> {
        toml::to_string(self)
            .map_err(|_| GtConfigError::FailedToStringify)
            .into_diagnostic()
    }
    ...
    pub fn to_toml_str_pruned(&self) -> Result<String> {
        ...
        let defaults_str = GtConfig::default().to_toml_str()?;
